...
/**
 * This is our implementation of the {@link GLG2DImageHelper}
 *
...
class RecordingImageHelper implements GLG2DImageHelper {
	...
	public boolean drawImage(Image img, int x, int y, int width, int height,
		...
		final double imgHeight = img.getHeight(null);
		final double imgWidth = img.getWidth(null);
	...
	public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2,
			...
			ImageObserver observer) {
//		final TextureEntry texture = textureCache.getTexture(img, observer);
	...
	protected boolean drawImage(Image img, AffineTransform xform, Color color,
		...
		final float height = img.getHeight(observer);
		final float width = img.getWidth(observer);
		if (width < 0 || height < 0) {
			return false;
		}
		final float[] points = new float[] { 0, height, // <-se
				width, height, // <-ne
				width, 0, // <-nw
				0, 0, // <-sw
		};
		...
		final RecordedGeometry rec = RecordedGeometry.generateTexture(texture,
				color.getRGB(), points, 0, 0, 1, 1);
